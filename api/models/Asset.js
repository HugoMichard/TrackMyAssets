...
Asset.getAssetsOwned = function (usr_id, result) {
  sql.query(
    `SELECT 
      ...
      INNER JOIN (
        SELECT ast_id, SUM(cast(quantity as double precision)) as quantity, SUM(cast(quantity as double precision) * price + fees) / SUM(cast(quantity as double precision)) as price FROM orders WHERE usr_id = ? GROUP BY ast_id
      ) owned_assets
